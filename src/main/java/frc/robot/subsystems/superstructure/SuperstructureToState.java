...
public class SuperstructureToState extends SequentialCommandGroup
{
  ...
  public SuperstructureToState(Superstructure superstructure, SuperState targetState)
  {
    ...
    Command shooterCmd = Commands.waitUntil(m_shooterWait)
                                 .andThen(superstructure.m_shooter.shootIt(m_targetState.shoot.speed)
                                                                  .until(m_shooterUntil));
    Command feederCmd = Commands.waitUntil(m_feederWait)
                                .andThen(superstructure.m_feeder.setSpeed(m_targetState.feed.power)
                                                                .until(m_feederUntil));
    Command elevatorCmd = Commands.waitUntil(m_elevatorWait)
                                  .andThen(superstructure.m_elevator.setAngle(m_targetState.elevator.angle)
                                                                    .until(m_elevatorUntil));
    Command intakeCmd = Commands.waitUntil(m_intakeWait)
                                .andThen(superstructure.m_intake.positionIntake(m_targetState.intake.position)
                                                                .until(m_intakeUntil));
    Command climberCmd = Commands.waitUntil(m_climberWait)
                                 .andThen(superstructure.m_climber.setHeight(m_targetState.climb.height)).until(
            m_climberUntil);
    // If the drivebase command is null then we send a null command

    ParallelCommandGroup commands = new ParallelCommandGroup(shooterCmd,
                                                             feederCmd,
                                                             elevatorCmd,
                                                             intakeCmd,
                                                             climberCmd);
  ...
  private void determineConditions()
  {
    ...
    if (m_targetState == SuperState.CLIMB_REACH)
    {
      m_climberWait = () -> (elevator.getAngle() >= (m_targetState.elevator.angle) &&
                             (intake.getIntakePistonPosition() == DoubleSolenoid.Value.kOff));
    }
